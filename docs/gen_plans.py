...
E_STAGES = [
    ...
    dict(
        ...
        outputs=["`src/models/state_head.py`（`q_joint + q_por/q_perm/q_sw`）、`src/inference/atomic_gate.py`", "`models/E6/pd1_*.pt`、`experiments/E6/P2/pd1/{result.json,result.zip,cv.json}`", "`reports/E6_gate.json`、`reports/E6_atomic_report.json`"],
...
"E0": [
 ("P0","环境与磁盘实测（云端第一次运行）",
  ...
  ["所有 hard 级检查全过（判据不是固定项数，也不因 `--allow-non-a100` 放宽依赖检查）；磁盘预算按挂载点报告且 Gate 的 `disk_budget_ok` 读 `--data-root`（`/data`）级别"],
 ...
 ("P3","提交契约、版本路由与干净目录冒烟",
  ...
  ["正/负样例都被正确判定；`python predict.py --list-versions` 在干净目录可用；E0 本地契约 Gate 12/12（含 cache）全部 mandatory 通过"],
