...
P["E4"] = [
    dict(
        ...
        stop=["资源预检耗时超过 E3 单折的 3 倍且无优势 → 判 NO-GO，保留 CNN 主干"],
...
P["E8"] = [
    dict(
        ...
        steps=["实现 MMoE：E 个专家 + 每任务独立门控 softmax 加权",
               ...
               "判定：至少一个目标提升且无目标退化",
               "胜者跑全 5 折并汇总 OOF"],
        params=[("专家数 E", "4", "2/4/8", "inner-OOF 选择"),
...
P["E10"] = [
    dict(
        ...
        goal="用 E9 通过候选的配置在全部 80 井上重训（或改用 5 折权重集成），"
             "导出 CPU 可加载的 fp32 `.pt` + `.npz` 权重清单（ONNX 为可选 best-effort）；"
             "验证两次前向完全一致。",
        ...
        steps=["按预注册选择聚合方式（全量重训 或 5 折权重集成）",
               ...
               "导出：`state_dict` → fp32 `.pt` + `.npz` 键名清单（主路径）",
               "ONNX 为 best-effort：仅当 `import onnx` 成功时才试 `torch.onnx.export`，"
               "否则**明确跳过并记录 degraded 原因**（torch 未装 onnx 时会抛 "
               "`OnnxExporterError: Module onnx is not installed!`，这不是失败）",
