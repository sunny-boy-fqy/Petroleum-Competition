...
**实测反驳**（80 口训练井，非缺测且非联合占位行；数字由 `tools/gen_data_card_md.py` 从 `reports/E0_data_card.json` 生成）：

| 目标 | min | p01 | median | p99 | max | <1 的行数 |
|---|---:|---:|---:|---:|---:|---:|
| POR | 0.0000 | 1.7420 | 11.3390 | 22.5507 | 33.1770 | **576** |
...
<!-- BEGIN:AUTO_STATS (由 tools/gen_data_card_md.py 生成，勿手改) -->

### 自动生成的统计表（来源：`reports/E0_data_card.json`）
...
| 有效行 | 236,343 | 0 |
| 非规范 schema 井 | 3 | 0 |
| 单井行数 min/median/max | 4473 / 9547 / 13078 | — |

#### 目标分布（训练井，按切片）

| 目标 | 切片 | n | min | p01 | median | p99 | max | <1 行数 |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| POR | 非缺测 | 723,567 | 0.0000 | 0.1000 | 0.1000 | 20.8283 | 33.1770 | — |
| POR | 仅有效行 | 236,331 | 0.0000 | 1.7420 | 11.3390 | 22.5507 | 33.1770 | 576 |
| PERM | 非缺测 | 723,557 | 0.0100 | 0.0100 | 0.0100 | 33.2450 | 1064.6830 | — |
| PERM | 仅有效行 | 236,331 | 0.0100 | 0.0100 | 0.8340 | 54.9964 | 1064.6830 | 122716 |
| SW | 非缺测 | 723,557 | 8.3050 | 41.9756 | 99.9000 | 99.9000 | 99.9000 | — |
| SW | 仅有效行 | 236,331 | 8.3050 | 32.0133 | 82.8050 | 99.9000 | 99.9000 | 0 |
...
自洽校验：`100×(0.30·POR + 0.35·PERM + 0.35·SW) = 70.4907350`，与上报 total 一致 = **True**（口径 `drop`）。
