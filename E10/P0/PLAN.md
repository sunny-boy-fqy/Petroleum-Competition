...
用 E9 通过候选的配置在全部 80 井上重训（或改用 5 折权重集成），导出 CPU 可加载的 fp32 `.pt` + `.npz` 权重清单（ONNX 为可选 best-effort）；验证两次前向完全一致。

## 2. 为什么需要这一步

1. `rules.md` §8.3 要求"训练 + 推理"可独立运行、结果一致；因此训练入口必须真实可跑，即使评测时不需要；
2. 评测机不保证有加速器，推理必须在 CPU 上稳定运行（fp32、确定性）；
...
- `models/v4/final/*.weights.npz`（键名/形状清单，torch 异常时的兜底）
- `models/v4/final/*.onnx`（**仅在用户额外装了 onnx 时**才产出）
...
3. 导出：`state_dict` → fp32 `.pt` + `.npz` 键名清单（主路径）
4. ONNX 为 best-effort：仅当 `import onnx` 成功时才试 `torch.onnx.export`，否则**明确跳过并记录 degraded 原因**（torch 未装 onnx 时会抛 `OnnxExporterError: Module onnx is not installed!`，这不是失败）
5. CPU 冒烟：加载 → 前向 → 与训练期预测逐点比对（容差 ≤1e-6）
6. 两次独立前向的 sha256 必须一致（确定性验证）
7. 记录峰值内存与单次推理耗时（目标 < 30 min、< 8 GiB）
...
- 在推理阶段做任何训练
- 把训练日志/中间产物写进提交包
- 把 ONNX 导出成功写成任何 Gate 的硬条件（默认不装 onnx）
...
| 未安装 onnx | ONNX 兜底不可用 | **预期行为**：主路径是 `.pt` + `.npz` 清单，跳过即可 |
...
```bash
# 云端（平台训练任务）
bash "$(find /code/workspace -name run_train.sh | head -1)" --mode stage --stage E10
# 本机（口径层，无 torch）
