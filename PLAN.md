...
>
> 同时已复算并冻结：训练 80 井 / **730,268 行**；测试 10 井 / **95,948 行**；状态计数 缺测 **6,700** / 占位 **487,225** / 有效 **236,343**；折指纹 sha256 `f7c2c58b…d94b87e`（80 井 / 5 折）。E0 Gate **6/6 mandatory 通过**（`reports/E0_gate.json`）。
...
| 层级 | 数量 | 篇幅 | 状态 |
|---|---:|---:|---|
| 总计划 `PLAN.md` | 1 | 661 行 | ✅ 完成 |
| 阶段计划 `E*/PLAN.md` | 12 | 每份 52–67 行 | ✅ 完成 |
| P 级子计划 `E*/P*/PLAN.md` | 33 | **平均 224 行**（合计约 7,600 行） | ✅ 完成（V2 深度：输入/输出契约、执行步骤、参数表、完成判据、禁止事项、风险对策、停止规则、复算命令、Gate 预注册要点） |
| 计划文件总行数 | 46 | **10,336 行** | ✅ |
...
**E0 Gate：6/6 mandatory PASS**（`reports/E0_gate.json`）。
...
> **H3 的尺度细节**：训练标签里 SW 占位是 `99.9`（百分数尺度），有效值是 `[0,1]`（小数尺度）——**同一列两种尺度**。这必须在 E0 数据卡里写死，并做单元测试：`SW` 的有效分支输出必须乘 100 才与占位分支同尺度，否则会掉约 10 分。这一条是 v4 最容易踩的坑。
...
- **⚠️ E0 实测发现（2026-09-19，本机复算，必须遵守）**：本地 80 口训练井中**有 3 口井的表头与官方 17 列不一致**，共 **27,080 行（3.71%）**：
  ...
  | 井（前 8 位） | 列数 | 差异 | 行数 |
  |---|---:|---|---:|
  | `42f2870b` | 20 | 多 `K, U, CGR`；**缺 `CASE`** | 7,879 |
  | `b7eb1274` | 21 | 多 `TH, K, U, CGR`；**缺 `CASE`** | 9,547 |
  | `c7611b01` | 16 | **缺 `CASE`** | 9,654 |
...
- 规范列顺序：`DEPTH, GR, PE, SP, CAL, AC, DEN, CNL, RXO, RT, DEVI, AZIM, BIT, CASE | POR, PERM, SW`；规范宽度 **17 列**（14 输入 + 3 目标），解析后必须断言 `arr.shape[1] == 17`。
...
| 组 | 内容 | 数量 | 阶段 |
|---|---|---|---|
| `F_raw` | 14 条原始曲线 | 14 | E1 |
| `F_miss` | 逐曲线缺失指示位 + 井内缺失比例 | 14 + 1 | E1 |
...
- [E0 数据、评测与提交契约](E0/PLAN.md) — 数据卡、哨兵、标签状态、评分器复算（70.4907）、按井折、提交契约与 CPU-only 单测。**不训练任何模型。**　**状态：已完成（E0 Gate 6/6 PASS，`reports/E0_gate.json`）。**
...
| Gate | 硬门槛 | 附加条件 |
|---|---|---|
| E0 | 数据卡可复算；评分器复算全常量基线 = **70.4907**（±1e-4）；折指纹一致；提交契约单测通过（**无需 torch**）；`check_env.py` 通过（Py2.4.0/cu12.6/A100）；`disk_budget_ok`（云端实测剩余 ≥ 8 GB） | 干净目录下 `python predict.py --help` 可运行 |
