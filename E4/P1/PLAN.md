...
把 CNN 主干与 PatchTF 的逐行表示按**门控融合或 FiLM**（其次 concat）融合后送同一组头，**融合头保持小容量**，结构与权重只在 inner-OOF 上选，判定是否超过**最佳单主干**；否则 NO-GO 并保留 E3 结构。
...
4. 改进 proposal §6：优先**门控 / FiLM** 而不是直接 concat（concat 会让融合头参数量膨胀并在 80 井上过拟合），且融合结构/权重只能由 inner-OOF 决定。
...
1. 实现三种融合：① **门控加权**（可学习标量/向量门）；② **FiLM**（逐通道仿射调制）；③ concat + 1×1（作对照）；受控保持融合头参数量小
2. 冻结两个主干的预训练权重先做快速筛查（只训融合层与头）
3. 胜出方案再解冻联合微调（小 lr）
4. 融合结构/权重只在 inner-OOF 上选（fold0 仅资源预检）
5. 与最佳单主干做同折 paired bootstrap
6. 写 Gate：融合 ≥ 最佳单主干且 CI 下界 > 0

## 6. 参数与配置

| 参数 | 默认值 | 搜索范围/说明 | 选择位置 |
|---|---|---|---|
| 融合方式 | 门控（默认） | 门控/FiLM/concat/平均 | inner-OOF 选（fold0 仅资源预检） |
